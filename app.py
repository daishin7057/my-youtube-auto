...
# --- 1. 하이엔드 다크 테마 디자인 (image_a3e91d.png 스타일) ---
st.set_page_config(page_title="YT Creator Studio Pro", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
    <style>
    .main { background-color: #0d1117; color: #e6edf3; }
    .stMetric { background-color: #161b22; padding: 25px; border-radius: 15px; border: 1px solid #30363d; border-left: 5px solid #3b82f6; }
    .ai-card { background: #161b22; padding: 20px; border-radius: 12px; border: 1px solid #30363d; text-align: center; transition: 0.3s; }
    ...
    .status-msg { background-color: #1e2130; color: #238636; padding: 12px; border-radius: 10px; text-align: center; font-weight: bold; border: 1px solid #238636; }
    </style>
...
# AI 엔진 인증 (image_a6333b.png 404 에러 원천 차단)
model = None
if "GEMINI_API_KEY" in st.secrets:
    try:
        ...
        st.error(f"AI 엔진 연결 오류: {e}")

# --- 2. 사이드바 (가이드 1단계 준수) ---
with st.sidebar:
    st.title("🎬 YT Studio Pro")
    st.caption("AI 영상 자동화 플랫폼 v20.0")
    menu = st.radio("🧭 메뉴 전환", ["🏠 대시보드", "✨ 콘텐츠 생성", "🤖 AI 검색엔진", "🔄 집/회사 동기화", "⚙️ 설정"])
    st.divider()
    st.markdown("<div class='status-msg'>🎉 프로그램이 정상적으로 작동합니다!</div>", unsafe_allow_html=True)

# --- 3. 메뉴별 기능 구현 ---

# [3-1] 대시보드: image_a3e91d.png의 웅장한 레이아웃
if menu == "🏠 대시보드":
    st.header("🏠 대시보드")
...
# [3-2] 콘텐츠 생성: image_a5b2ba.png 버그 수정 및 초정밀 타임라인
elif menu == "✨ 콘텐츠 생성":
    ...
    with col_t2: 
        # image_a5b2ba.png 에러 방지: 최대값을 59로 엄격히 제한
    ...
    if st.button("⚡ 전체 자동 생성 가동"):
        if topic and model:
            ...
            try:
                res = model.generate_content(f"{topic} 주제로 {duration} 분량 대본과 {style} 프롬프트 생성.")
...
# [3-4] 동기화 및 설정
    ...
    st.download_button("📤 데이터 내보내기", data=data, file_name=f"yt_studio_backup_{datetime.now().strftime('%m-%d')}.json")
